...
<div class="max-w-4xl mx-auto bg-white rounded-xl border border-slate-200 p-6">
    <h1 class="text-2xl font-bold text-slate-900 mb-6">নতুন ব্যয়</h1>

    <form action="{{ route('expenses.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-5">
        @csrf
        <div>
            <label class="block text-sm font-medium mb-1">তারিখ</label>
            <input type="date" name="expense_date" value="{{ old('expense_date', now()->toDateString()) }}" class="w-full border rounded-lg px-3 py-2 focus:ring-2 focus:ring-amber-300">
        </div>
        <div>
        ...
        <div class="md:col-span-2 flex justify-end gap-3">
            <a href="{{ route('expenses.index') }}" class="px-4 py-2 rounded-lg border border-slate-300">বাতিল</a>
